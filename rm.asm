...
main(int argc, char *argv[])
{
  ...
  exit();
    ...
    1050:	50                   	push   %eax
    1051:	ff 33                	push   (%ebx)
    1053:	68 8c 17 00 00       	push   $0x178c
    1058:	6a 02                	push   $0x2
    ...
    13ac:	89 d1                	mov    %edx,%ecx
{
  ...
  do{
    ...
    13d9:	0f b6 92 04 18 00 00 	movzbl 0x1804(%edx),%edx
    ...
    1459:	8b 5d 0c             	mov    0xc(%ebp),%ebx
{
  ...
  for(i = 0; fmt[i]; i++){
    ...
    14c9:	ff 24 85 ac 17 00 00 	jmp    *0x17ac(,%eax,4)
        putc(fd, c);
      }
      state = 0;
    }
  ...
  write(fd, &c, 1);
    ...
    15d7:	90                   	nop
          s = "(null)";
    15d8:	ba a5 17 00 00       	mov    $0x17a5,%edx
        while(*s != 0){
...
free(void *ap)
{
  ...
  for(p = freep; !(bp > p && bp < p->s.ptr); p = p->s.ptr)
    15f1:	a1 bc 1a 00 00       	mov    0x1abc,%eax
{
  ...
  freep = p;
    ...
    1631:	5b                   	pop    %ebx
  freep = p;
    1632:	89 15 bc 1a 00 00    	mov    %edx,0x1abc
}
    1638:	5e                   	pop    %esi
    1639:	5f                   	pop    %edi
    163a:	5d                   	pop    %ebp
  ...
  freep = p;
    166c:	89 15 bc 1a 00 00    	mov    %edx,0x1abc
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
  ...
  if((prevp = freep) == 0){
    168c:	8b 3d bc 1a 00 00    	mov    0x1abc,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
  ...
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    ...
    if(p == freep)
    16d1:	8b 3d bc 1a 00 00    	mov    0x1abc,%edi
  ...
  return freep;
    16ff:	8b 15 bc 1a 00 00    	mov    0x1abc,%edx
    ...
    172c:	89 70 04             	mov    %esi,0x4(%eax)
      freep = prevp;
    172f:	89 15 bc 1a 00 00    	mov    %edx,0x1abc
}
    ...
    173f:	c3                   	ret    
    base.s.ptr = freep = prevp = &base;
    1740:	c7 05 bc 1a 00 00 c0 	movl   $0x1ac0,0x1abc
    1747:	1a 00 00 
    base.s.size = 0;
    174a:	bf c0 1a 00 00       	mov    $0x1ac0,%edi
    base.s.ptr = freep = prevp = &base;
    174f:	c7 05 c0 1a 00 00 c0 	movl   $0x1ac0,0x1ac0
    1756:	1a 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
    1759:	89 fa                	mov    %edi,%edx
    base.s.size = 0;
    175b:	c7 05 c4 1a 00 00 00 	movl   $0x0,0x1ac4
    1762:	00 00 00 
    if(p->s.size >= nunits){
